...
static double varerr(int sat, int sys, double el, double bl, double dt, int f,
    const prcopt_t* opt, double wm, double wiu, double wir)
{
    ...
    a = fact * opt->err[1]; /* su MDP o KOU moltiplicare per peso calcolato in precedenza (MDP solo per rover, KOU per rover e reference). Su CON sostituire peso */
    b = fact * opt->err[2];

    if (opt->gter_im_model == 2 && wiu > 0 && wir > 0)
        a = b = MAX(wiu, wir); /* Ionospheric weighting with Conker - verify if the choice of the largest value is correct! */

    if (opt->gter_im_model == 1 && opt->gter_mdp_enable)
        highMultWeigh = MAX(MAX(wiu, wir), wm);
    else if (opt->gter_im_model == 1)
        highMultWeigh = MAX(wiu, wir);
    else if (opt->gter_mdp_enable)
        highMultWeigh = wm;

    if (highMultWeigh <= 0) highMultWeigh = 1; /* do not weight observables if weights are not available */
                      ...
                      const prcopt_t *opt, double *y, double *wm, double* wi, double *freq)
{
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* iono-free linear combination */
        ...
        for (i=0;i<nf;i++) {
            ...
            if (wi != NULL)
            {
                wi[i] = obs->ionoWeight[i];
                wi[i+nf] = obs->ionoWeight[i+NFREQ];
            }
        }
    }
}
...
static int zdres(int base, const rtk_t* rtk, const obsd_t *obs, int n, const double *rs,
                 ...
                 int index, double *y, double *wm,  double *wi, 
                 double *e, double *azel, double *freq)
{
    ...
    /* warning! test only 
    pos[0] = -22.123075 * PI / 180.0;
    pos[1] = -51.407990 * PI / 180.0;
...
static int ddres_mod(rtk_t *rtk, const nav_t *nav, double dt, const double *x,
                 ...
                 int ns, double *v, double *H, double *R, int *vflg, int* hubs, int* iuValid, int* irValid)
{
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        if (i<0) continue; /* i = index of the hub satellite */
        ...
        for (j=0;j<ns;j++) {
            ...
            Ri[nv]=varerr(sat[i],sysi,azel[1+iu[i]*2],bl,dt,f,opt,wm!=NULL?wm[iu[i]*nf+obsIdx]:-1.0,wi!=NULL?wi[iu[i]*2*nf+f]:-1.0,wi!=NULL?wi[ir[i]*2*nf+f]:-1.0); /* wm[iu[i]*nf+f]: MDP weight of the hub satellite (i) on rover station (u)*/
            Rj[nv]=varerr(sat[j],sysj,azel[1+iu[j]*2],bl,dt,f,opt,wm!=NULL?wm[iu[j]*nf+obsIdx]:-1.0,wi!=NULL?wi[iu[j]*2*nf+f]:-1.0,wi!=NULL?wi[ir[j]*2*nf+f]:-1.0); /* wm[iu[j]*nf+f]: MDP weight of the non-hub satellite (j) on rover station (u)*/
...
{
    return ddres_mod(rtk, nav, dt, x, P, sat, y, NULL, NULL, e, azel, freq, iu, ir, ns, v, H, R, vflg, NULL, NULL, NULL);
}
                  ...
                  const nav_t *nav)
{
    ...
    if (stat!=SOLQ_NONE&&zdres(0,rtk,obs,nu,rs,dts,var,svh,nav,xp,opt,0,y,NULL,NULL,e,azel,freq)) {
    ...
    if (stat==SOLQ_FLOAT) {
        ...
        if (manage_amb_LAMBDA(rtk,bias,xa,sat,nf,ns)>1) {
            ...
            if (zdres(0,rtk,obs,nu,rs,dts,var,svh,nav,xa,opt,0,y,NULL,NULL,e,azel,freq)) {
